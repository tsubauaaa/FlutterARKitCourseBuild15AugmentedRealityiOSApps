...
class _ArMeasurementScreenState extends State<ArMeasurementScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(
          title: const Text('Distance Tracker App'),
        ),
        body: Container(
  ...
  void onARViewCreated(ARKitController arKitController) {
    ...
    _arKitController.onAddNodeForAnchor = addAnchor;
    _arKitController.onUpdateNodeForAnchor = updateAnchor;
    _arKitController.onARTap = (List<ARKitTestResult> ar) {
      final ARKitTestResult? planeTap = ar.firstWhereOrNull(
  ...
  void onPlaneTapHandler(Matrix4 transform) {
    ...
    final material = ARKitMaterial(
      ...
      diffuse: ARKitMaterialProperty.color(
        const Color.fromRGBO(255, 153, 83, 1),
      ),
    );
  ...
  void drawText(String textDistance, vector.Vector3 point) {
    ...
    _arKitController.getNodeBoundingBox(node).then(
      (List<vector.Vector3> result) {
